...
async function checkAndRenew(env, isSched, lang = "zh") {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        // ============================================================
        if (iAutoRenew && days <= -Math.abs(iRenewDays)) {
            // 定时任务运行时，检查是否到达当天最晚的提醒时间
            ...
            if (isSched) {
                ...
                ntArr.forEach(nt => {
                    ...
                    const totalMinutes = tgtH * 60 + tgtM;
                    if (totalMinutes > latestMinute) {
                        latestMinute = totalMinutes;
                    }
                });
                // 只有在最晚提醒时间前后5分钟内才执行自动续期
                if (latestMinute >= 0) {
                    const currentMinutes = nowH * 60 + nowM;
                    shouldRenew = Math.abs(currentMinutes - latestMinute) <= 5;
                }
            }
        ...
        if (days <= iNotifyDays) {
            ...
            if (isSched) {
                ...
                // 如果服务启用了自动续期且当前是最晚提醒时间点，不推送到期提醒（续期提醒会覆盖）
                if (shouldPush && iAutoRenew && days <= -Math.abs(iRenewDays)) {
                    let latestMinute = -1;
                    ntArr.forEach(nt => {
                        const [tgtH, tgtM] = String(nt).split(":").map(Number);
                        const totalMinutes = tgtH * 60 + tgtM;
                    ...
                    const currentMinutes = nowH * 60 + nowM;
                    if (latestMinute >= 0 && Math.abs(currentMinutes - latestMinute) <= 5) {
                        shouldPush = false;
                    }
                }
            }
            ...
            if (shouldPush) {
                trig.push({ ...st, note: msg, fixedPrice: it.fixedPrice, currency: it.currency, renewUrl: it.renewUrl, tip: it.tip });
            } else {
                monitor.push({ ...st });
            }
        } else {
    ...
    if (s.enableNotify) {
        ...
        if (allChannels.length === 0) {
            ...
            for (const ch of allChannels) {
                ...
                // Build unified push body
                const allItems = [...chDis, ...chUpd, ...chTrig];
                if (allItems.length > 0) {
                    allItems.forEach((x) => {
                        // 天数状态
                        let dayStatus = '';
                        if (x.daysLeft > 0) {
                            dayStatus = `(还有${x.daysLeft}天)`;
                        } else if (x.daysLeft < 0) {
                            dayStatus = `(逾期${Math.abs(x.daysLeft)}天)`;
                        }
                        ...
                        if (x.fixedPrice) {
                            itemBody += `\n\n帐单：¥${x.fixedPrice}${x.currency ? ` ${x.currency}` : ''}`;
                        }
                        if (x.renewUrl) {
                            itemBody += `\n\n链接：\`${x.renewUrl}\``;
                        }
                        if (x.tip) {
                            const tips = x.tip.split('\n').filter(line => line.trim());
                            if (tips.length > 0) {
                        ...
                        pushTasks.push(
                            Notifier.dispatch([ch], title, itemBody)
                                .then(res => `[${ch.name}]: ${res}`)
                        );
                    });
                }
