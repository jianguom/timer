...
const settingsForm = ref({
    ...
    customTitle: 'Timer',
    customSubtitle: '守时中心',
    customEnglishSlogan: 'Service Lifecycle Management',
    customChineseDescription: '分布式云资产全周期托管中枢',
    customCopyrightYear: '2025-2026',
    customDesigner: 'Lucas',
    customBackgroundUrl: ''
});
...
const getNearestNotifyTime = (item) => {
    const notifyTimes = Array.isArray(item.notifyTimes) ? item.notifyTimes : (item.notifyTime ? [item.notifyTime] : ['08:00']);
    const nextDueDate = item.nextDueDate;
    if (!nextDueDate) return null;

    const now = new Date();
    const todayStr = getLocalToday();
    const isToday = nextDueDate === todayStr;
    const [dueY, dueM, dueD] = nextDueDate.split('-').map(Number);
    const sortedTimes = [...notifyTimes].sort();
    let targetDate = null;
    ...
    if (isToday) {
        ...
        for (const timeStr of sortedTimes) {
            ...
            const t = new Date(dueY, dueM - 1, dueD, h, m);
            if (t > now) {
                targetDate = t;
                break;
            }
        ...
        if (!targetDate) {
            ...
            const nextDay = new Date(dueY, dueM - 1, dueD + 1);
            targetDate = new Date(nextDay.getFullYear(), nextDay.getMonth(), nextDay.getDate(), h, m);
        }
    } else {
        // 非当天：显示到期日最早时间
        const [h, m] = sortedTimes[0].split(':').map(Number);
        targetDate = new Date(dueY, dueM - 1, dueD, h, m);
    }
...
const updatePageTitle = () => {
    const customTitle = settings.value?.customTitle || 'RenewHelper';
    const title = document.title;
    const match = title.match(/v(\d+\.\d+\.\d+)/);
    if (match) {
        document.title = `${customTitle} v${match[1]}`;
    } else {
        ...
        <el-config-provider :locale="locale">
            ...
            <div v-else class="max-w-7xl mx-auto w-full">
                    ...
                    <div class="flex items-center gap-5 self-start lg:self-center">
                        ...
                        <div class="flex flex-col">
                            <div class="flex items-baseline flex-wrap gap-x-3 gap-y-1">
                                <h1 class="text-4xl font-black tracking-tighter text-textMain">{{ settings.customTitle || 'RenewHelper' }}</h1>
                                ...
                                <span class="text-2xl font-bold text-blue-600 tracking-wider"
                                    style="font-family: 'Microsoft YaHei', sans-serif;">{{ settings.customSubtitle || '守时中心' }}</span>
                            ...
                            <div class="flex items-center mt-1 flex-wrap gap-2">
                                <p
                                    ...
                                    {{ settings.customEnglishSlogan || 'Service Lifecycle Management' }}</p>
                                ...
                                <p class="text-[10px] text-gray-500 font-bold tracking-[0.1em] whitespace-nowrap"
                                    style="font-family: 'Microsoft YaHei', sans-serif;">{{ settings.customChineseDescription || '分布式云资产全周期托管中枢' }}</p>
                            </div>
                        </div>
                    </div>
                                ...
                                <template #default="scope">
                                    <div v-if="scope.row.enabled">
                                        ...
                                        <div
                                            ...
                                            <span>TARGET: {{ getNearestNotifyTime(scope.row) || scope.row.nextDueDate }}</span>
                ...
                <div class="mt-8 py-6 text-center border-t border-slate-200/60">
                    <p
                        ...
                        &copy; {{ settings.customCopyrightYear || '2025-2026' }} <a href="https://github.com/ieax/renewhelper" target="_blank"
                            class="font-bold text-slate-600 hover:text-blue-600 transition-colors border-b border-dashed border-slate-300 hover:border-blue-600 pb-0.5 mx-1 decoration-0">{{ settings.customTitle || 'RenewHelper' }}</a>
                        ...
                        <span class="mx-2 opacity-30">|</span>DESIGNED BY <span
                            class="font-bold text-slate-600">{{ settings.customDesigner || 'LOSTFREE' }}</span>
                    </p>
                </div>
            </div>
            ...
            <el-dialog v-model="dialogVisible" :title="isEdit ? t('editService') : t('newService')" width="680px"
                ...
                <el-form :model="form" label-position="top">
                    ...
                    <el-form-item :label="lang === 'zh' ? '小贴士' : 'Tip'"><el-input v-model="form.tip" type="textarea"
                            rows="3" :placeholder="lang === 'zh' ? '支持 Markdown 格式：**粗体** *斜体* `代码` ~~删除线~~ [链接](url)' : 'Supports Markdown: **bold** *italic* `code` ~~strikethrough~~ [link](url)'"></el-input></el-form-item>
            ...
            <el-dialog v-model="settingsVisible" :title="t('settingsTitle')" width="800px" align-center
                ...
                <div class="flex h-[550px] text-left border-t border-slate-100 dark:border-slate-800 overflow-hidden">
                    ...
                    <div
                        ...
                        <div v-for="tab in ['pref', 'customize', 'notify', 'calendar', 'data']" :key="tab"
                            ...
                            <el-icon v-else-if="tab === 'customize'">
                                <Monitor />
                            </el-icon>
                            ...
                            <span class="hidden sm:block">
                                {{ tab === 'pref' ? (lang === 'zh' ? '偏好设置' : 'Preferences') :
                                    tab === 'customize' ? (lang === 'zh' ? '信息设置' : 'Info') :
                    ...
                    <div
                        ...
                        <div class="flex-1 p-4 scrollbar-thin overflow-y-auto overflow-x-hidden">
                            ...
                            <!-- 2. Info Settings -->
                            <div v-if="activeTab === 'customize'" class="space-y-4">
                                <h3
                                    class="text-base font-bold text-slate-800 dark:text-gray-100 mb-3 pb-2 border-b border-gray-100 dark:border-slate-800 flex items-center gap-2">
                                    {{ lang === 'zh' ? '信息设置' : 'Info Settings' }}
                                </h3>
                                <el-form :model="settingsForm" label-position="top">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <el-form-item :label="lang === 'zh' ? '应用标题' : 'App Title'">
                                            <el-input v-model="settingsForm.customTitle" placeholder="Timer"></el-input>
                                        </el-form-item>
                                        <el-form-item :label="lang === 'zh' ? '副标题' : 'Subtitle'">
                                            <el-input v-model="settingsForm.customSubtitle" placeholder="守时中心"></el-input>
                                        </el-form-item>
                                        <el-form-item :label="lang === 'zh' ? '英文标语' : 'English Slogan'">
                                            <el-input v-model="settingsForm.customEnglishSlogan" placeholder="Service Lifecycle Management"></el-input>
                                        </el-form-item>
                                        <el-form-item :label="lang === 'zh' ? '中文描述' : 'Chinese Description'">
                                            <el-input v-model="settingsForm.customChineseDescription" placeholder="分布式云资产全周期托管中枢"></el-input>
                                        </el-form-item>
                                        <el-form-item :label="lang === 'zh' ? '版权年份' : 'Copyright Year'">
                                            <el-input v-model="settingsForm.customCopyrightYear" placeholder="2025-2026"></el-input>
                                        </el-form-item>
                                        <el-form-item :label="lang === 'zh' ? '设计者' : 'Designer'">
                                            <el-input v-model="settingsForm.customDesigner" placeholder="Lucas"></el-input>
                                        </el-form-item>
                                        <el-form-item :label="lang === 'zh' ? '背景图URL' : 'Background URL'" class="sm:col-span-2">
                                            <el-input v-model="settingsForm.customBackgroundUrl" placeholder="https://example.com/bg.jpg" type="url"></el-input>
                                            <p class="text-xs text-gray-400 mt-1">{{ lang === 'zh' ? '支持 jpg、png、webp 格式' : 'Supports jpg, png, webp formats' }}</p>
                                        </el-form-item>
                                    </div>
                                </el-form>
                            </div>
            ...
            <el-dialog v-model="renewDialogVisible"
                ...
                <div v-if="currentRenewItem.tip" class="tip-card" v-html="parseMarkdown(currentRenewItem.tip)"></div>
                <template #footer>
